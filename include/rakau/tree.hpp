...
public:
    ...
    template <typename It>
    explicit tree(frip f, const F &box_size, bool box_size_deduced, const std::array<It, NDim + 1u> &cm_it,
                  const size_type &N, const size_type &max_leaf_n)
        : m_box_size(box_size), m_box_size_deduced(box_size_deduced), m_max_leaf_n(max_leaf_n), m_ncrit(0)
    {
        simple_timer st("overall tree construction");
        // Param consistency checks: if size is deduced, box_size must be zero.
        assert(!m_box_size_deduced || m_box_size == F(0));
        ...
        }
        {
            ...
            // TODO figure out the chunking stuff.
            for (std::size_t j = 0; j < NDim + 1u; ++j) {
                tbb::parallel_for(tbb::blocked_range<size_type>(0u, N, 10000000),
        ...
        }
        {
            ...
            tbb::parallel_for(tbb::blocked_range<size_type>(0u, N), [this](const auto &range) {
                ...
                for (auto i = range.begin(); i != range.end(); ++i) {
                    // Compute and store the code.
                    disc_coords(tmp_dcoord, i);
                    m_codes[i] = ph_encode<cbits>(tmp_dcoord);
                }
            });
        }
        // Do the sorting of m_isort.
        indirect_code_sort(m_isort.begin(), m_isort.end());
        {
            // TODO: probably decrease parallelism here to avoid thrashing, but measure.
            // Apply the permutation to the data members.
            // These steps can be done in parallel.
            simple_timer st_p("permute");
            tbb::task_group tg;
    ...
    void build_tree_new()
    {
        ...
        // NOTE: we will be moving around in the codes vector using random access iterators. Thus, we must ensure the
        // difference type of the iterator can represent the size of the codes vector.
        using it_diff_t = typename std::iterator_traits<decltype(m_codes.begin())>::difference_type;
        using it_udiff_t = std::make_unsigned_t<it_diff_t>;
        if (m_codes.size() > static_cast<it_udiff_t>(std::numeric_limits<it_diff_t>::max())) {
            throw std::overflow_error("The number of particles (" + std::to_string(m_codes.size())
        ...
        struct code_shifter {
            explicit code_shifter(UInt s) : m_s(s) {}
            auto operator()(UInt c) const
            {
                return static_cast<UInt>(c >> m_s);
            }
            UInt m_s;
        };

        simple_timer st2("barfer");

        // Do the root node manually.
        const auto root_sqr_node_dim = m_box_size * m_box_size;
        if (!std::isfinite(root_sqr_node_dim)) {
            throw std::invalid_argument(
                "The computation of the square of the dimension of the root node leads to the non-finite value "
        ...
        m_tree.push_back(node_type{size_type(0),
                                   ...
                                   1,
                                   0,
                                   // NOTE: make sure the node properties are inited to zero.
                                   {},
                                   root_sqr_node_dim});

        UInt cur_level = 1;
        size_type node_added_prev_level = 1;
        std::vector<node_type> node_buffer;

        while (true) {
            if (!node_added_prev_level || cur_level > cbits) {
                break;
            }
            // std::cout << "Current level: " << cur_level << '\n';

            const auto t_begin
                = boost::make_transform_iterator(m_codes.begin(), code_shifter(NDim * (cbits - cur_level))),
                t_end = boost::make_transform_iterator(m_codes.end(), code_shifter(NDim * (cbits - cur_level)));
            const auto prev_level = static_cast<UInt>(cur_level - 1u);
            // TODO overflow checks?
            const auto max_new_nodes = static_cast<size_type>(node_added_prev_level * (UInt(1) << NDim));
            // std::cout << "Max new nodes: " << max_new_nodes << '\n';
            // TODO overflow checks?
            node_buffer.resize(max_new_nodes);
            const auto old_tree_size = m_tree.size();
            tbb::parallel_for(
                tbb::blocked_range<decltype(m_tree.size())>(old_tree_size - node_added_prev_level, old_tree_size),
                [&](const auto &range) {
                    for (auto i = range.begin(); i != range.end(); ++i) {
                        // std::cout << "Parent node index: " << i << '\n';
                        // Extract a reference to the parent node.
                        const auto &parent_node = m_tree[i];
                        const auto start_b_idx = (i - (old_tree_size - node_added_prev_level)) * (UInt(1) << NDim);
                        // std::cout << "Starting index in the buffer: " << start_b_idx << '\n';
                        if (parent_node.end - parent_node.begin <= m_max_leaf_n) {
                            // std::cout << "Parent node has too few particles, it will be a leaf\n";
                            for (UInt j = 0; j < (UInt(1) << NDim); ++j) {
                                node_buffer[start_b_idx + j].dim2 = -1;
                            }
                        } else {
                            const auto parent_code = parent_node.code;
                            // std::cout << "parent code: " << parent_code << '\n';
                            const auto s_code
                                = static_cast<UInt>((parent_code - (UInt(1) << (prev_level * NDim))) << NDim);
                            // std::cout << "s_code: " << s_code << '\n';
                            for (UInt j = 0; j < (UInt(1) << NDim); ++j) {
                                // std::cout << "target shifted code: " << j + s_code << '\n';
                                const auto r = std::equal_range(t_begin + parent_node.begin, t_begin + parent_node.end,
                                                                static_cast<UInt>(j + s_code));
                                const auto node_nparts = static_cast<size_type>(std::distance(r.first, r.second));
                                if (node_nparts) {
                                    // std::cout << "Found node with " << node_nparts << " particles\n";
                                    node_buffer[start_b_idx + j]
                                        = node_type{// Node begin.
                                                    static_cast<size_type>(std::distance(t_begin, r.first)),
                                                    // Node end.
                                                    static_cast<size_type>(std::distance(t_begin, r.second)),
                                                    // Number of children.
                                                    0,
                                                    // Code.
                                                    (UInt(1) << (cur_level * NDim)) + j + s_code,
                                                    // Level.
                                                    cur_level,
                                                    // Node properties (inited to zero).
                                                    {},
                                                    get_sqr_node_dim(cur_level)};
                                } else {
                                    // std::cout << "Found empty node\n";
                                    node_buffer[start_b_idx + j].dim2 = -1;
                                }
                            }
                        }
            ...
            node_added_prev_level = 0;

            for (auto &n : node_buffer) {
                if (n.dim2 > 0) {
                    m_tree.push_back(std::move(n));
                    ++node_added_prev_level;
                }
            }

            ++cur_level;

            node_buffer.resize(0);
        }

        std::cout << "Tree size: " << m_tree.size() << '\n';
