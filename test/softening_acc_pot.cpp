...
TEST_CASE("accelerations/potentials softening ordered")
{
                    ...
                    for (auto ncrit : ncrits) {
                        for (auto eps : softs) {
                            ...
                            if (eps != fp_type(0)) {
                                ...
                                t.accs_pots_u(
                                    {accpots[0].data(), accpots[1].data(), accpots[2].data(), accpots[3].data()}, theta,
                                    kwargs::eps = eps, kwargs::split = sp);
...
TEST_CASE("accelerations/potentials softening unordered")
{
    tuple_for_each(macs{}, [](auto mac_type) {
        tuple_for_each(fp_types{}, [mac_type](auto x) {
            using fp_type = decltype(x);
